...
def get_fb_token():
    token_usuario = obtener_token(PLATAFORMA)["token_acceso"]
    return obtener_token_pagina(token_usuario, FB_PAGE_ID)
...
def fb_get_fb(path, params=None):
    """Wrapper para fb_get con token dinámico."""
    return fb_get(path, params or {}, access_token=get_fb_token())

def fb_paginate(path, params=None):
    """Wrapper para paginate con token dinámico."""
    return paginate(path, params or {}, access_token=get_fb_token())
...
def get_posts_por_rango(inicio: date, fin: date):
    """Descarga publicaciones dentro del rango [inicio, fin]."""
    since = inicio - timedelta(days=1)
    fields = ",".join([
        "id", "created_time", "message", "permalink_url", "status_type",
        "attachments{media_type,unshimmed_url}",
...
# ==========================================
# Función principal (para CLI y Airflow)
# ==========================================
def run_fb_ingest(start_date: str, end_date: str):
    """Función principal reutilizable (para Airflow o CLI)."""
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        run_fb_ingest(sys.argv[1], sys.argv[2])
    else:
        hoy = date.today().isoformat()
        run_fb_ingest(hoy, hoy)
