...
def main():
    args = sys.argv
    if len(args) < 2:
        print("Uso: python fb_ig_ingest_unificado.py [facebook|instagram|ambos] [inicio opcional] [fin opcional]")
        sys.exit(1)
    modo = args[1].lower()
    if len(args) == 4:
        inicio = datetime.strptime(args[2], "%Y-%m-%d").date()
        fin = datetime.strptime(args[3], "%Y-%m-%d").date()
    else:
    ...
    if modo == "facebook":
        ...
        print("Modo no reconocido, usa: facebook | instagram | ambos")
        sys.exit(1)
    calcular_variaciones()
    logger.info("✔ Ingesta y variaciones completadas correctamente")
